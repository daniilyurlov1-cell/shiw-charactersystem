...
local DurabilityConfig = {
    ...
    checkInterval = 60000,
    --- Один швейный набор полностью восстанавливает прочность (до maxDurability).
...
-- Принудительно снять одежду, если её отдают другому игроку через инвентарь
-- Используется rsg-inventory при переносе clothing_* из инвентаря игрока в otherplayer-XXX
RegisterNetEvent('rsg-clothing:server:OnGivenToOtherPlayer', function(playerId, fromSlot)
    ...
    -- Снимаем метку equipped и обновляем кеш экипировки
    ...
    -- Убираем одежду с персонажа и синхронизируем в БД
    TriggerClientEvent('rsg-clothing:client:removeClothing', src, category)
    SyncClothesToDatabase(src)
...
--- Снять 1 швейный набор без указания слота: у предметов из БД часто нет/не совпадает item.slot — RemoveItem(..., slot) тогда не находит запись.
...
RegisterNetEvent('rsg-clothing:server:repairFromInventory', function(slot)
    ...
    -- Ремонтируем до 100% (SetItemDurability мутирует item в Player.PlayerData.items[slot])
    local newDurability = DurabilityConfig.maxDurability
    SetItemDurability(item, newDurability)
    
    -- ★ Синхронизируем инвентарь с клиентом (НЕ вызывать GetInventory — он перезапишет изменения)
    Player.Functions.SetInventory(Player.PlayerData.items)
...
RSGCore.Functions.CreateCallback('rsg-clothing:server:getEquippedClothing', function(source, cb)
    local ok, err = pcall(function()
        ...
        -- ★ 1) ОСНОВНОЙ ИСТОЧНИК: напрямую из БД playerskins.clothes
        ...
        -- ★ 2) Fallback: если по какой‑то причине БД пустая, добираем из инвентаря/кэша
        if next(equippedClothes) == nil then
            local items = Player.PlayerData.items or {}

            -- сначала — инвентарь по data.equipped
            ...
            -- затем — server‑кэш playerEquippedClothing
            if playerEquippedClothing[src] then
                for category, equipped in pairs(playerEquippedClothing[src]) do

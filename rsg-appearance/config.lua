...
-- ★ Отключение дебаг-вывода. Поставьте true для отладки.
RSG.Debug = false

-- Бандана поднята (/bandana из neckwear): не рисовать меш на локальном педе в камере от первого лица
RSG.HideBandanaMeshInFirstPerson = true
-- Если FP не детектится: значения режима follow-cam с клиента (по умолчанию 4).
RSG.FirstPersonCamViewModes = { 4 }

--- Если Z игрока <= MaxZForTeleport, пробуем поправить высоту по коллизии. Иначе позицию не трогаем.
RSG.UnderMapSafety = {
    --- false = полностью отключить (для кастомных маппингов с низким Z или нестандартной «землёй»)
    Enabled = true,
    MaxZForTeleport = -7.0,
    --- false = если земля не найдена, оставить координаты от сервера (не телепорт в FallbackCoords)
    UseFallbackCoords = false,
    --- Используется только при UseFallbackCoords = true
    FallbackCoords = vector4(-275.72, 807.28, 119.38, 90.0),
    --- Перед GetGroundZ подгрузить коллизию в точке (снижает ложные срабатывания при раннем входе)
    WaitForCollisionMs = 2500,
}

--[[
  ricx_outfits / кастомные куртки: «просветы» на рукавах и спине
  Чаще всего это НЕ ширина тела: узкий меш рукава, плохие vertex weights или то,
  как движок деформирует ткань с двумя винтовками на спине. Поджатие тела (slim)
  редко помогает; правка — другой предмет в пресете, правка ассета (Blender/OpenIV),
  или смена позиции/набора оружия на спине (holster-скрипты).
]]
RSG.RicxOutfitSlimBody = {
    -- true = при активном костюме ricx поджимать MP-тело (может не убрать дыры, но исказит пропорции)
    enabled = false,
}

-- Снять MP-меш тела под ricx-форму. custom_id = имя предмета в инвентаре (outfit_id_N) = WearOutfitWithCustomID.
-- Верх: как shirts_full. Перчатки gloves_000 только в ricxBareHandsAfterUpperHideCustomIds (тон кожи + tint на слоте).
RSG.RicxOutfitHideBodyMesh = {
    enabled = true,
    hideLower = true,
    hideUpper = true,
    ricxHideUpperBodyCustomIds = {
        -- Полиция жен. 1–2, полиция 1–5
        ...
        -- Одеяние челонианца (предмет outfit_id_51)
        'outfit_id_51',
        -- Комплект выживальщика 1–5
        'outfit_id_52', 'outfit_id_53', 'outfit_id_54', 'outfit_id_55', 'outfit_id_56',
        -- Комплект ранчера 2, 4
    ...
    ricxBareHandsAfterUpperHideCustomIds = {
        'outfit_id_29', 'outfit_id_34',
        'outfit_id_42',
        -- HS: без overlay кисти пропадают вместе с BODIES_UPPER; HS 7 с перчатками в пресете — overlay не ставится (кэш gloves)
        'outfit_id_44', 'outfit_id_45', 'outfit_id_46', 'outfit_id_47', 'outfit_id_48', 'outfit_id_49', 'outfit_id_50',
        'outfit_id_51',
        'outfit_id_58', 'outfit_id_60',
        'outfit_id_63', 'outfit_id_66',
...
-- Переопределяем print для подавления дебаг-вывода
...
-- Опционально: цены на конкретные модели
RSG.ItemPrices = {
    ['coats'] = {
        [35] = {
            [1] = 30,  -- красный

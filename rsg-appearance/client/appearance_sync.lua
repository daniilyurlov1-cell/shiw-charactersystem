--[[
    ★ Синхронизация внешности другим игрокам
    Применяет head, face features, overlays и одежду к педам других игроков
    на основе Player State Bag (appearance).
]]
...
-- ★ Дополняем только «полный» скин из БД/JSON. Если таблица обрезана (мало ключей), нули вместо отсутствующих
-- морфов делают лицо шире/другим, чем у владельца (например face_width: локально -40, на чужом клиенте 0).
...
-- Применить внешность к педу другого игрока (без изменения глобального состояния локального игрока)
-- ★ После ApplyClothes вызывается UpdatePedVariation — он сбрасывает лицо/оверлеи.
-- Делаем повторное применение лица (как для своего персонажа), иначе у других лицо другое.
local function ApplyAppearanceToOtherPed(targetPed, skinData, clothesData)
    ...
    -- ★ Обязательно: подбородок, нос, глаза — иначе лицо у других случайное
    ...
    -- ★ КРИТИЧНО: ApplyClothes/UpdatePedVariation сбрасывают лицо (подбородок, нос, глаза) и оверлеи
    SetTimeout(1200, function()
        ...
        -- UpdatePedVariation сбрасывает face features и overlays — применяем заново
        Wait(50)
        LoadFeatures(targetPed, skinData)
        LoadOverlays(targetPed, skinData)
    end)
...
-- ★ Полный скин с сервера (JSON) — надёжнее state bag для face features у чужих педов
...
-- ★ State Bag: для чужих — приоритет skin_json (полные морфы); таблица skin часто урезана
AddStateBagChangeHandler('appearance', nil, function(bagName, key, value)
    ...
    -- Чужой игрок: не крутим обрезанный skin — только JSON или ждём PeerAppearancePayload
...
-- ★ Применяем внешность при первом появлении игрока (state мог быть установлен до нашего подключения)
CreateThread(function()
    while true do
        Wait(2000)
        local myServerId = GetPlayerServerId(PlayerId())

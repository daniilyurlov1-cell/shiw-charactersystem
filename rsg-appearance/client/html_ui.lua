...
function OpenCreatorHTML(step)
    ...
    -- ★ Редактирование существующего персонажа: подставляем полный скин из LoadedComponents в CreatorCache,
    -- чтобы при сохранении не терялись параметры (голова, оверлеи, черты лица и т.д.)
...
RegisterNUICallback('confirmCreator', function(data, cb)
    ...
    -- ★ Режим editappearance: только скин, валидация имени не нужна
    ...
    -- ★ Режим «редактор без сброса»: применяем только скин к текущему педу, не трогаем модель и одежду
    if KeepClothesOnSave then
        local playerPed = PlayerPedId()
        local isMale = Selectedsex == 1
        -- Удаляем педа создания и показываем игрока
        if CreatorPed and DoesEntityExist(CreatorPed) then
            DeleteEntity(CreatorPed)
            CreatorPed = nil
        end
        ...
        -- Возврат на место, откуда открыли /editappearance
    ...
    -- Применяем одежду только если не режим «без сброса» (админ-редактор)
    if not KeepClothesOnSave then
    ...
    if Skinkosong then
        ...
        if EnsureFullCreatorCache then EnsureFullCreatorCache() end
        if KeepClothesOnSave then
            -- Режим «редактор без сброса»: сохраняем только скин, одежду не трогаем
            TriggerServerEvent('rsg-appearance:server:SaveSkin', CreatorCache, {}, true)
            KeepClothesOnSave = false
        else
...
RegisterNUICallback('cancelCreator', function(data, cb)
    ...
    -- ★ Режим /editappearance: возвращаем на место, не в меню выбора персонажа

...
-- ★ Режим «редактор без сброса»: при сохранении не трогать одежду, только скин
KeepClothesOnSave = false
-- ★ Позиция возврата после /editappearance: { x, y, z, h }
...
-- ★ Глубокое копирование таблицы (для сохранения всех параметров скина в LoadedComponents)
...
-- ★ Добавляем недостающие ключи в CreatorCache со значениями 0 — чтобы при загрузке не подставлялись дефолты
-- Вызывать перед SaveSkin, чтобы в БД сохранялись ВСЕ face features и overlays
...
-- ★ Макияж: UpdatePedVariation (0xCC8CA3E88256E58F) сбрасывает overlays — переприменяем после смены одежды/анимаций
...
-- ★ Только одежда (без head/hair/eyes) — для loadcharacter: раздеваем перед LoadClothingFromInventory
...
function SpawnPeds()
    ...
    -- ★ FIX: Не поднимать игрока на z+2 — при вылете в создателе сервер может сохранить эту позицию и игрок спавнится на крыше
    SetEntityCoords(playerPed, coords.x, coords.y, coords.z, false, false, false, false)

    -- ★ Режим editappearance: CreatorCache уже заполнен из скина, не перезаписываем
...
function ApplyCreatorComponents(ped, gender, cache)
    ...
    -- ★ Только первые 20 лиц: ограничиваем список голов (20 × 6 тонов = 120)
...
--- Только при Z <= порога: при необходимости подгружаем коллизию и ищем опору по тем же X/Y.
--- Без UseFallbackCoords позиция сервера сохраняется — нет телепорта в Валентайн при «не нашли землю».
local function TeleportToSafeGroundIfNeeded(ped)
    ...
    -- С неба вниз — при глубоком провале coords.z+50 всё ещё под картой
    local groundZ = nil
...
AddEventHandler('RSGCore:Client:OnPlayerLoaded', function()
    ...
    -- ★ Под карта при раннем спавне (до ApplySkin): несколько отложенных проверок
...
local function ScheduleAppearanceRecoveryPasses(skinData)
    ...
    -- ★ Позднее переприменение overlays (12 сек) — защита от сброса сторонними скриптами
    SetTimeout(12000, function()
        ...
        if ped and DoesEntityExist(ped) and not IsEntityDead(ped) and LoadOverlays then
            ...
            -- ★ После UpdatePedVariation корсет сбрасывается — переприменяем при _PedStateFrozen
    ...
    -- ★ Переприменяем лицо и overlays через 5.5 сек — исправляет "слетание" шрамов, морщин, макияжа
    -- ★ Выполняем ВСЕГДА (даже при _PedStateFrozen): UpdatePedVariation/одежда сбрасывают overlays
...
AddEventHandler('rsg-appearance:client:ApplySkin', function(skinData, clothesData)
    ...
    -- ★ Защита от спама loadcharacter: повторный вызов во время загрузки игнорируем,
    -- иначе второй вызов читает HP уже нового педа (полное) и "восстанавливает" его
    if LocalPlayer.state.isLoadingCharacter then
        return
    end

    -- ★ Нормализуем ключи из БД (могут прийти строками из JSON) — иначе LoadHead/пол дают неверный результат
    ...
    -- ★ Полная копия скина из БД в LoadedComponents, чтобы при сохранении из редактора не терялись параметры
    ...
    local applySkinWhenDead = isMetaDead  -- после рестарта в ноке — применить скин, в конце вернём HP 0
    ...
    -- ★ FIX: Удаляем пропы, прикреплённые к педу (доски rsg-construction и т.д.) — loadcharacter ломает педа при attached prop
    -- ★ Не удаляем зонтики (shiw-parasol)
    ...
    -- ★ Женский mp_female: базовый мета-пресет до тела/лица — иначе лицо в игре не совпадает с редактором
    ...
    -- ★ loadcharacter: Спавнить голого → одеть из БД (playerskins.clothes) или из инвентаря
    -- ★ SetPedPortFromSkin уже вызван в ApplyAndSaveBodyMorph → ApplyAllBodyMorph (порт/вариация).
    -- Повторный вызов здесь сбрасывал слои лица и оверлеи до одевания — «плоское» лицо в игре vs редактор.
    ...
    if type(clothesData) == 'table' and next(clothesData) then
        -- Одежда из БД (playerskins.clothes) — применяем напрямую (hash уже разрешён на сервере)
    ...
    -- ★ UpdatePedVariation (при загрузке одежды) сбрасывает голову и оверлеи — переприменяем лицо полностью
    -- ★ skipBodyMorph=true — не переприменяем ReapplyBodyMorph после одежды (фикс вылезания на полных)
    ped = PlayerPedId()
    ...
    -- ★ Body morph: LoadHead+UpdatePedVariation сбрасывают его — восстанавливаем. ReapplyVestIfEquipped убран — жилет уже в equipOrder.
    ped = PlayerPedId()
    ApplyAndSaveBodyMorph(ped, skinData)
    -- ApplyAllBodyMorph снова вызывает SetPedPortFromSkin → UpdatePedVariation — сбрасывает морщины/ageing/текстуру лица.
    ped = PlayerPedId()
    if DoesEntityExist(ped) then
        LoadHead(ped, skinData)
        Wait(40)
    ...
    -- ★ НЕ запускаем BodyMorphGuard — после loadcharacter пед замораживается (_PedStateFrozen), морф не трогаем
    ...
    -- ★ Только если Z <= UnderMapSafety.MaxZForTeleport (под картой); обычные интерьеры не трогаем.
    TeleportToSafeGroundIfNeeded(ped)

    -- ★ Сначала _PedStateFrozen — иначе между сбросом isLoadingCharacter и этим флагом срабатывает
    -- scheduleOverlayRefreshAfterAnim (TaskPlayAnim при экипировке) и LoadOverlays идёт в другом потоке
    -- параллельно с хвостом ApplySkin → глобальное состояние оверлеев ломается и loadcharacter «умирает».
    ...
    -- ★ УБРАНО: finalBodyMorphPass на 800мс/1800мс — вызывал клиппинг корсета (тело менялось после одежды)

    -- ★ При _PedStateFrozen: ноги + корсет. Запускаем ПОСЛЕ прохода 5500ms (LoadHead/UpdatePedVariation), иначе тот проход сбрасывает корсет и он просвечивает
    SetTimeout(6000, function()
        ...
        if _G._PedStateFrozen then
            ...
            local cache = nil
            pcall(function() cache = exports['rsg-appearance']:GetClothesCache() end)
            -- 1) Сначала скрываем ноги (жен.: штаны/юбка/платье+обувь)
            pcall(function()
                exports['rsg-appearance']:ApplyLegsStateForSkirtBoots(currentPed, cache)
            end)
            Wait(150)
            -- 2) Потом переприменяем корсет/жилет (как из инвентаря), чтобы он не слетал после снятия ног
            if cache then
...
-- ★ Когда одежда загружена из инвентаря, переприменяем лицо (голова/черты/оверлеи) — UpdatePedVariation сбрасывает их
-- ★ FIX: НЕ запускаем при _PedStateFrozen (после loadcharacter) — иначе каждое надевание шляпы/одежды
-- через 500мс переприменяло лицо+NativeUpdatePedVariation и сбрасывало только что надетое
AddEventHandler('rsg-clothing:client:clothingLoaded', function()
    if _G._PedStateFrozen then return end
    SetTimeout(500, function()
        if _G._PedStateFrozen then return end
        -- ★ Пальто: LoadHead+NativeUpdatePedVariation вызывали мигание корсета
        ...
        if data and next(data) then
            ...
            Wait(50)
            -- ★ Не вызываем ReapplyBodyMorph после смены одежды (фикс вылезания на полных)
...
-- ★ Админ: открыть редактор внешности без сброса (текущий скин загружается, одежда не меняется)
-- Телепорт в комнату создания, после сохранения — возврат на место.
RegisterNetEvent('rsg-appearance:client:OpenEditor', function()
    local ped = PlayerPedId()
    if not DoesEntityExist(ped) or IsEntityDead(ped) then
        lib.notify({ title = 'Ошибка', description = 'Нельзя открыть редактор в этом состоянии', type = 'error' })
        return
    end
    RSGCore.Functions.TriggerCallback('rsg-appearance:server:GetSkinForEditor', function(skin)
        if not skin or not next(skin) then
            lib.notify({ title = 'Ошибка', description = 'Данные внешности не найдены', type = 'error' })
            return
        end
        -- Сохраняем позицию для возврата после сохранения
        ...
        -- Запускаем создателя: телепорт в комнату, пед, затем открываем только визуал (без одежды/данных)
        StartCreator()
        lib.notify({ title = 'Редактор внешности', description = 'Можно менять только лицо и тело, одежда не изменится', type = 'info' })
    end)
end)
...
exports('GetBodyCurrentComponentHash', function(name)
    ...
    if name == "hair" or name == "beard" then
        ...
        -- Поддержка hair_hashname / beard_hashname (как в LoadHair)
